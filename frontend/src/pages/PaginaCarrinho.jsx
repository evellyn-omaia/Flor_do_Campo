...
function PaginaCarrinho() {
  ...
  return <><Cabecalho /><main className="pagina-carrinho carrinho-referencia">
    ...
    {carregandoAuth || carregandoCarrinho ? <div className="carrinho-vazio"><p>Carregando seu carrinho...</p></div> : !usuario ?
      ...
      </section><aside className="coluna-resumo"><div className="resumo-pedido"><h2>Resumo do pedido</h2><p>{itens.reduce((s,i) => s + i.quantidade, 0)} itens</p><dl><div><dt>Subtotal</dt><dd>{dinheiro(subtotal)}</dd></div>{desconto > 0 && <div><dt>Desconto</dt><dd>− {dinheiro(desconto)}</dd></div>}<div><dt>Retirada</dt><dd className="preco-rosa">Na loja</dd></div></dl><div className="resumo-total"><b>Total</b><strong>{dinheiro(total)}</strong></div><Link className="finalizar-compra" to="/checkout"><i className="ri-secure-payment-line"/> Finalizar compra</Link></div>
    ...
    <section className="faixa-servicos"><div><Icone nome="loja"/><span><b>Retirada na loja</b><small>Após a confirmação do pedido</small></span></div><div><i className="ri-loop-left-line servico-icone"/><span><b>Troca fácil e gratuita</b><small>Até 7 dias após a compra</small></span></div><div><i className="ri-shield-check-line servico-icone"/><span><b>Compra 100% segura</b><small>Seus dados protegidos</small></span></div><div><i className="ri-bank-card-line servico-icone"/><span><b>Parcelamento em até 6x</b><small>Sem juros no cartão</small></span></div></section>
  </main><Rodape /></>;
}
